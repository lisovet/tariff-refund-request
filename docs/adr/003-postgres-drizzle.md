...
- **Postgres** as the system of record. Neon for dev/preview environments (branching DBs per PR), AWS RDS or Neon prod tier for production.
...
- ✅ Neon branching means every preview deploy gets a fresh DB.
- ⚠️ Drizzle's ecosystem is smaller than Prisma's — accept some rough edges.

## Test-impact

- **No DB mocking, ever.** Integration tests use a real Postgres (Neon branch in CI, or testcontainers locally).
